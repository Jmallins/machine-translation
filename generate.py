...
def get_args():
    ...
    parser.add_argument('--output', default=None, type=str, help='maximum number of sentences in a batch')
...
def main(args):
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, num_workers=1, collate_fn=test_dataset.collater,
        batch_sampler=BatchSampler(
            test_dataset, args.max_tokens, args.batch_size, 1,
            0, shuffle=False, seed=args.seed))
    #print (os.path.join(args.data, 'test.{}'.format(args.source_lang)))
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i, (sample_id, hypos) in enumerate(zip(sample['id'].data, hypos)):
            ...
            for i, hypo in enumerate(hypos[:min(len(hypos), args.num_hypo)]):
                ...
                allhyps[int(sample_id)] = hypo_str
                if not args.quiet:
    ...
    if args.output is not None:
        ...
        for i in range(len(allhyps.keys())):
                       f.write (allhyps[i]+"\n")
        f.close()
